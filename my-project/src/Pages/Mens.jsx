          ...
          <div className="sm:flex sm:flex-row sm:w-[100%] sm:gap-4 sm:h-auto sm:flex-wrap sm:justify-start ">
            ...
            {all_product.map((product) => {
              ...
              if (product.category === "men") {
                return (
                  <Link to={`/Product/${product.id}`} key={product.id} className='md:w-[240px] sm:w-[100px] 
                  sm:h-auto sm:space-y-4 sm:p-4 sm:flex sm:flex-col'>
                      <img className='hover:scale-[115%] cursor-pointer ease-in-out duration-1000' 
                      src={product.image} alt="" srcset="" />
                      <div className=' sm:flex sm:flex-row sm:justify-between'>
                        <p className='text-black sm:text-xs'>{product.name}</p>
                        <p className='text-black sm:text-xs'>{product.new_price}$</p>
                      </div>
                  </Link>
                );
              }
